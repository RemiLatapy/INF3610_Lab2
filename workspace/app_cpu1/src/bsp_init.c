...
int connect_irqs() {
	...
	status = connect_stopServiceTaskHandler();
	if (status != XST_SUCCESS)
		return XST_FAILURE;


	status = connect_receptionTaskHandler();
	if (status != XST_SUCCESS)
		return XST_FAILURE;
...
int connect_statisticTaskHandler()
{
	 ...
	 * interrupt occurs for the device.
	 */
	status = XIntc_Connect(&m_axi_intc, PRINT_STATS_IRQ_ID, (Xil_ExceptionHandler)irq_gen_1_isr, NULL);
	if (status != XST_SUCCESS)
		return status;
	 ...
	 * Enable the interrupt
	 */
	XIntc_Enable(&m_axi_intc, PRINT_STATS_IRQ_ID);
...
int connect_stopServiceTaskHandler()
{
	 ...
	 * Enable the interrupt
	 */
	XIntc_Enable(&m_axi_intc, FIT_1S_IRQ_ID);
...
int connect_receptionTaskHandler()
{
	 ...
	 * interrupt occurs for the device.
	 */
	status = XIntc_Connect(&m_axi_intc, RECEIVE_PACKET_IRQ_ID, (Xil_ExceptionHandler)irq_gen_0_isr, NULL);
	if (status != XST_SUCCESS)
		return status;
	 ...
	 * Enable the interrupt
	 */
	XIntc_Enable(&m_axi_intc, RECEIVE_PACKET_IRQ_ID);
...
int connect_verificationTaskHandler()
{
	 ...
	 * interrupt occurs for the device.
	 */
	status = XIntc_Connect(&m_axi_intc, FIT_5S_IRQ_ID, (Xil_ExceptionHandler)fit_timer_5s_isr, NULL);
	if (status != XST_SUCCESS)
		return status;
	 ...
	 * Enable the interrupt
	 */
	XIntc_Enable(&m_axi_intc, FIT_5S_IRQ_ID);
