...
@********************************************************************************************************
    ...
    .global  ARM_ExceptAddrAbortHndlr
...
OS_CPU_ARM_ExceptHndlr_BreakTask_1:
    ...
    LDR     R1, =IRQInterrupt                             		@ IRQInterrupt(except_type = R0);
    MOV     LR, PC
    BX      R1
...
OS_CPU_ARM_ExceptHndlr_BreakExcept:
    ...
    MSR     CPSR_cxsf, R3                                       @ RESTORE INTERRUPTED MODE.
                                                                ...
                                                                @ EXECUTE EXCEPTION HANDLER:
    LDR     R2, =IRQInterrupt                             		@ IRQInterrupt(except_type = R0);
    MOV     LR, PC
    BX      R2
...
OS_CPU_ARM_IRQHndlr_BreakIRQ:
    ...
    STR     SP, [R3]
                                                                ...
                                                                @ EXECUTE EXCEPTION HANDLER:
    LDR     R3, =IRQInterrupt                             		@ IRQInterrupt(except_type = R0)
    MOV     LR, PC
    BX      R3
...
@
@ Note(s) : 1) OS_CPU_SR_INT_En() can be called by IRQInterrupt() AFTER the external
...
@                                          ENABLE & DISABLE IRQs
@
@ Note(s) : 1) OS_CPU_SR_IRQ_En() can be called by IRQInterrupt() AFTER the external
...
@                                          ENABLE & DISABLE FIQs
@
@ Note(s) : 1) OS_CPU_SR_FIQ_En() can be called by IRQInterrupt() AFTER the external
